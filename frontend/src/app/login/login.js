document.addEventListener('DOMContentLoaded', function() {
        ...
        if (validateForm()) {
            ...
            setTimeout(function() {
                ...
                if (success) {
                    ...
                    // 模擬跳轉至首頁
                    setTimeout(function() {
                        window.location.href = 'dashboard.html'; // 跳轉至儀表板頁面
                    }, 1500);
                } else {
    ...
    forgotPasswordLink.addEventListener('click', function(e) {
        e.preventDefault();
        showModal('忘記密碼', `
            <p>請輸入您的電子郵件，我們將發送重置密碼連結。</p>
            <div class="form-group" style="margin: 20px 0;">
                <input type="email" class="form-input" id="reset-email" placeholder="您的電子郵件">
            </div>
            <button class="login-button" id="send-reset-btn">發送重置連結</button>
        `);
        ...
        document.getElementById('send-reset-btn').addEventListener('click', function() {
            ...
            closeModal();
            showNotification('密碼重置連結已發送至您的電子郵件', 'success');
        });
    });
