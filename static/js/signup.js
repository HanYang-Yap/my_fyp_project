...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.verifyOTP = function () {
        ...
        .then(data => {
            if (data.message) {
                ...
                document.getElementById('otpError').textContent = data.error || '驗證碼錯誤';
                document.getElementById('otpError').style.display = 'block';
            }
        })
        .catch(() => alert('驗證錯誤，請稍後再試'));
    }
    ...
    async function submitFormData() {
        ...
        try {
            ...
            const formData = {
                user_id: document.getElementById('email').value,  // 這裡改為使用 email 當作 user_id
